...
function StoreBannerInfo({ storeDetail }: StoreBannerInfoProps) {
  const { data: storeStaffList } = useQuery({
    queryKey: ["storeStaffs", { storeId: storeDetail?.id }],
    queryFn: () => getAllStoreStaffAPI({ storeId: storeDetail?.id }),
    select: (res) => res.data,
    enabled: !!storeDetail?.id,
  });
...
export default memo(StoreBannerInfo);
